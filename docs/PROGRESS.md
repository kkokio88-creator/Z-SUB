...
### 2026-02-13 세션 5 (실제 시트 데이터 통합)

**배경:** Google Sheets 실 데이터 연결 요청 (반찬 시트 + 식단\_히스토리)

**완료된 작업 (5개):**

1. **반찬 시트 CSV 파싱** - 215개 실제 메뉴 아이템 추출
   - 11개 카테고리(무침/메인/볶음/조림/밥류/국/전/협업/김치/장아찌/시즌국) → 4개 MenuCategory 매핑
   - `_parseSheetData.cjs` 파서 생성 → `data/realMenuDB.ts` 3447줄 생성
2. **TargetType 확장** - 10개 → 15개 타겟 타입
   - 추가: 첫만남/유아/든든한유아/어린이/든든한어린이
   - 식단\_히스토리 시트의 실제 타겟 타입과 일치
3. **constants.ts 리팩토링** - MOCK_MENU_DB → 실제 데이터 교체
   - 45개 하드코딩 메뉴 → 215개 실제 메뉴 (realMenuDB import)
   - 주재료 6종 → 10종 확장 (달걀/버섯/채소 추가)
   - 5개 신규 TARGET_CONFIGS 추가
4. **번들 최적화** - html2pdf 동적 import + 코드 스플리팅
   - `services/exportService.ts`: static → dynamic import('html2pdf.js')
   - `vite.config.ts`: menu-data(51KB) + vendor-pdf(984KB) 별도 청크
5. **Playwright UI 검증** - 15개 타겟 드롭다운, 실제 메뉴 식단 생성 확인

**커밋:**

- `8c9bb04` - feat: 실제 반찬 시트 데이터 통합 - 215개 메뉴 + 15개 타겟 타입
...
### 2026-02-13 세션 6 (식단 히스토리 달력 뷰)

**배경:** 세션 5 종료 후 미커밋 상태로 남아 있던 식단 히스토리 기능 복구 및 커밋

**완료된 작업 (6개):**

1. **식단 히스토리 컴포넌트** - `MealPlanHistory.tsx` (468줄)
   - 달력형 월별 뷰 (6주 × 7일 그리드)
   - 날짜별 식단 프리뷰 (대표 메뉴 3개 + 타겟/품목 수)
   - 화수목/금토월 사이클 배지
   - 상세 뷰: 타겟별 카드 그리드 + 이전/다음 네비게이션
   - 15개 타겟 필터 드롭다운
2. **히스토리 데이터** - `historicalMealPlans.ts` (21,481줄)
   - 2024-11-05 ~ 2026-12-30 실제 식단 134건
   - 10개 타겟 타입별 메뉴 구성 포함
3. **타입 정의** - HistoricalMealPlan, HistoricalTargetPlan, HistoricalMenuItem
4. **앱 통합** - 사이드바 탭 + React.lazy 라우팅 + history-data 청크 분리
5. **빌드 경고 해결** - chunkSizeWarningLimit 1000KB (lazy-loaded 청크 허용)
6. **임시 파일 정리** - .gitignore에 파싱 스크립트/스크린샷 추가

**커밋:**

- `0100dd2` - feat: 식단 히스토리 달력 뷰 추가 - 134개 날짜 실데이터
- `3d0ef40` - chore: 빌드 경고 제거 + 임시 파일 gitignore 추가
