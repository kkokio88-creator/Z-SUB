...
const AuditEntryDetail: React.FC<Props> = ({ entry, onBack }) => {
  ...
  return (
    <div className="h-full flex flex-col">
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-sm text-stone-600 hover:text-stone-800 mb-4 transition-colors"
      >
      ...
      <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
        ...
        <div className="p-6 border-b border-stone-100 bg-stone-50/50">
          <h3 className="text-lg font-bold text-stone-800">{entry.entityName}</h3>
        ...
        <div className="p-6 grid grid-cols-2 gap-4 border-b border-stone-100">
          <div>
            <label className="text-xs font-bold text-stone-400 uppercase">Entity Type</label>
            <p className="text-sm text-stone-800 mt-1">{entry.entityType}</p>
          </div>
          <div>
            <label className="text-xs font-bold text-stone-400 uppercase">Entity ID</label>
            <p className="text-sm text-stone-800 mt-1 font-mono">{entry.entityId}</p>
          </div>
        </div>
        ...
        {(entry.before || entry.after) && (
          <div className="p-6">
            <h4 className="text-sm font-bold text-stone-700 mb-4">변경 상세</h4>
        ...
        {entry.metadata && Object.keys(entry.metadata).length > 0 && (
          <div className="p-6 border-t border-stone-100">
            <h4 className="text-sm font-bold text-stone-700 mb-3">메타데이터</h4>
            <pre className="bg-stone-50 border border-stone-200 rounded-lg p-4 text-xs font-mono overflow-auto max-h-40 whitespace-pre-wrap">
