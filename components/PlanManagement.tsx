...
import { Settings, Plus, Save, Trash2, AlertCircle, X, Percent, PlusCircle, Minus, ArrowDownRight, Layers, LayoutList } from 'lucide-react';
...
// Convert constant record to array for state management
const INITIAL_CONFIGS: ManagedMealPlanConfig[] = Object.entries(TARGET_CONFIGS).map(([key, config]) => ({
  id: key,
  ...config
}));

const PlanManagement: React.FC = () => {
  const [configs, setConfigs] = useState<ManagedMealPlanConfig[]>(INITIAL_CONFIGS);
  const [globalCostRatio, setGlobalCostRatio] = useState<number>(30); // Default Global Cost Ratio
    ...
    setConfigs(prev => prev.map(config => {
      if (config.id === id) {
        ...
        if (field === 'targetPrice' || field === 'targetCostRatio') {
            const price = field === 'targetPrice' ? (parseInt(value) || 0) : config.targetPrice;
            const ratio = field === 'targetCostRatio' ? (parseFloat(value) || 0) : config.targetCostRatio;
            updatedConfig.budgetCap = Math.round(price * (ratio / 100));
        }

        return updatedConfig as ManagedMealPlanConfig;
      }
      return config;
    }));
  ...
  const handleApplyGlobalRatio = () => {
    if (window.confirm(`모든 식단의 원가율을 ${globalCostRatio}%로 일괄 적용하시겠습니까?`)) {
      setConfigs(prev => prev.map(config => ({
        ...config,
  ...
  const handleAddBasePlan = () => {
    const newId = `BASE_${Math.random().toString(36).substr(2, 5).toUpperCase()}`;
  ...
  const handleAddOptionPlan = (parentId: string, parentTarget: TargetType) => {
    const newId = `OPT_${Math.random().toString(36).substr(2, 5).toUpperCase()}`;
    const newPlan: ManagedMealPlanConfig = {
      id: newId,
  ...
  const handleDeletePlan = (id: string) => {
    if (window.confirm('정말 삭제하시겠습니까? 하위 옵션 식단이 있다면 함께 관리해야 할 수 있습니다.')) {
  ...
  const renderCompositionStepper = (config: ManagedMealPlanConfig, category: MenuCategory, label: string, colorClass: string) => (
      ...
      <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            ...
            <div className="flex items-center gap-3">
               ...
               <button className="flex items-center gap-2 px-5 py-2.5 bg-gray-900 text-white font-medium rounded-lg hover:bg-black shadow-md transition-all active:scale-95">
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-gray-50">
                            ...
                            <tr className="bg-white">
                                <td className="p-4">
                                    <span className="inline-block px-2 py-0.5 mb-1 rounded text-[10px] font-bold bg-gray-800 text-white">기본 (Base)</span>
                                ...
                                <td className="p-4">
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <div className="pt-1 mt-1 border-t border-gray-100 flex justify-between">
                                            ...
                                            <span className="text-xs font-bold text-primary-600">{base.budgetCap.toLocaleString()}원</span>
                                        </div>
                                    </div>
                                </td>
                                <td className="p-4">
                                    <div className="space-y-2">
                                        <div className="flex flex-wrap gap-1">
                                            {base.requiredTags.map(tag => (
                                                <span key={tag} className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-blue-50 text-blue-700 border border-blue-100">
                                                    {tag} <button onClick={() => handleRemoveTag(base.id, 'requiredTags', tag)}><X className="w-3 h-3 ml-1" /></button>
                                                </span>
                                            ))}
                                            <input 
                                                type="text" 
                                                ...
                                                onKeyDown={(e) => e.key === 'Enter' && (handleAddTag(base.id, 'requiredTags', e.currentTarget.value), e.currentTarget.value='')}
                                            />
                                        </div>
                                        <div className="flex flex-wrap gap-1">
                                            {base.bannedTags.map(tag => (
                                                <span key={tag} className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-red-50 text-red-700 border border-red-100">
                                                    {tag} <button onClick={() => handleRemoveTag(base.id, 'bannedTags', tag)}><X className="w-3 h-3 ml-1" /></button>
                                                </span>
                                            ))}
                                            <input 
                                                type="text" 
                                                ...
                                                onKeyDown={(e) => e.key === 'Enter' && (handleAddTag(base.id, 'bannedTags', e.currentTarget.value), e.currentTarget.value='')}
                                            />
                                        </div>
                                    </div>
                                </td>
                            ...
                            {options.map((opt) => (
                                <tr key={opt.id} className="bg-gray-50/50 hover:bg-blue-50/30 transition-colors">
                                    <td className="p-4 pl-8 relative">
                                        ...
                                        <span className="inline-block px-2 py-0.5 mb-1 rounded text-[10px] font-bold bg-white border border-gray-200 text-gray-500">옵션 (Option)</span>
                                        <input 
                                            type="text" 
                                            value={opt.target}
                                        ...
                                        <div className="space-y-1">
                                             <div className="flex flex-wrap gap-1">
                                                {opt.requiredTags.map(tag => (
                                                    <span key={tag} className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-blue-100 text-blue-800">
                                                        {tag} <button onClick={() => handleRemoveTag(opt.id, 'requiredTags', tag)}><X className="w-3 h-3 ml-1" /></button>
                                                    </span>
                                                ))}
                                                 <input 
                                                    type="text" 
                                                    ...
                                                    onKeyDown={(e) => e.key === 'Enter' && (handleAddTag(opt.id, 'requiredTags', e.currentTarget.value), e.currentTarget.value='')}
                                                />
                                            </div>
                                        </div>
                                    </td>
                            ...
                            {options.length === 0 && (
                                <tr className="bg-gray-50/30">
                                    <td colSpan={5} className="p-4 text-center text-xs text-gray-400 border-t border-dashed border-gray-200">
