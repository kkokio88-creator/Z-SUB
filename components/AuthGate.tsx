...
const AuthGate: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-screen bg-gradient-to-br from-stone-50 via-emerald-50/30 to-stone-100">
        <div className="flex flex-col items-center gap-4">
          <RefreshCw className="w-8 h-8 text-primary-600 animate-spin" />
          <p className="text-stone-500 text-sm font-medium">시스템 초기화 중...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-sm">
        <Card className="p-8">
          <div className="flex items-center justify-center gap-2 mb-6">
            ...
            <span className="text-2xl font-bold text-stone-800">Z-SUB</span>
          </div>
          <h2 className="text-center text-lg font-bold text-stone-800 mb-2">로그인</h2>
