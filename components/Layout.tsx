...
import { LayoutDashboard, Utensils, Users, Settings, LogOut, Leaf, Database, Layers } from 'lucide-react';
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col flex-shrink-0 z-20">
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50/50">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-gray-700">강지예 (MASTER)</p>
              <p className="text-xs text-gray-500">최고 관리자</p>
            </div>
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-8 flex-shrink-0 z-10 shadow-sm">
          <div>
            <h1 className="text-xl font-bold text-gray-800 flex items-center gap-2">
              {activeTab === 'dashboard' && <><LayoutDashboard className="w-6 h-6 text-gray-400"/> 통합 운영 대시보드</>}
              {activeTab === 'planner' && <><Utensils className="w-6 h-6 text-gray-400"/> 지능형 식단 생성</>}
              {activeTab === 'master-data' && <><Database className="w-6 h-6 text-gray-400"/> 기준 정보 관리</>}
              {activeTab === 'subscribers' && <><Users className="w-6 h-6 text-gray-400"/> 구독자 CRM</>}
              {activeTab === 'settings' && <><Settings className="w-6 h-6 text-gray-400"/> 시스템 설정</>}
            </h1>
          </div>
