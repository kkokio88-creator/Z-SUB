...
import { TrendingUp, Truck, Users, CheckCircle, CalendarClock, AlertTriangle, ArrowUpRight, ArrowDownRight, Activity, DollarSign, CreditCard, PieChart as PieChartIcon, Download } from 'lucide-react';
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, Legend, ScatterChart, Scatter, ZAxis, ReferenceLine, LineChart, Line, ComposedChart, Bar, BarChart } from 'recharts';

// --- Mock Data ---

// 1. Weekly Aggregate Trend
const WEEKLY_TREND_DATA = [
  { name: '3주전', subscribers: 820 },
  { name: '2주전', subscribers: 835 },
  { name: '지난주', subscribers: 856 },
  { name: '이번주', subscribers: 892 },
  { name: '다음주(예상)', subscribers: 910 },
];

// 2. Cycle Distribution
const CYCLE_DISTRIBUTION = [
  { name: '화수목 (A조)', value: 520 },
  { name: '금토월 (B조)', value: 372 },
];
const COLORS = ['#3b82f6', '#10b981', '#f59e0b', '#94a3b8'];

// 3. Plan-specific Trends (Multi-line)
...
// 5. Weekly Ops Data (Moved from SubscriberManagement)
         ...
         {activeTab === 'overview' && (
             <div className="flex-1 max-w-2xl bg-white border-l-4 border-l-red-500 rounded-r-xl shadow-sm p-3 flex items-center gap-4">
                ...
                <div className="flex-1">
                    ...
                    <span className="font-bold text-gray-900">'시니어 식단'</span> 이탈률 <span className="text-red-600 font-bold">8.5% (주의 필요)</span> 경고
                    </p>
                </div>
      ...
      {activeTab === 'overview' ? (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <StatCard title="이번 주 배송 예정" value="2,480건" trend="12%" trendUp={true} icon={Truck} colorClass="bg-blue-500 text-blue-600" />
                <StatCard title="주간 활성 구독자" value="892명" trend="4.2%" trendUp={true} icon={Users} colorClass="bg-green-500 text-green-600" />
                <StatCard title="식단 만족도" value="4.8/5.0" trend="0.1" trendUp={true} icon={CheckCircle} colorClass="bg-purple-500 text-purple-600" />
                <StatCard title="주간 예상 매출" value="1,240만원" trend="1.2%" trendUp={false} icon={TrendingUp} colorClass="bg-yellow-500 text-yellow-600" />
            </div>
        ...
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div className="flex items-center mt-2 text-xs font-bold text-green-600"><ArrowUpRight className="w-3 h-3 mr-1" /> +10.3%</div>
                </div>
                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                    <p className="text-sm font-bold text-gray-500 mb-1">영업 이익</p>
                    <h3 className="text-2xl font-bold text-gray-900">2,200만원</h3>
                    <div className="flex items-center mt-2 text-xs font-bold text-green-600"><TrendingUp className="w-3 h-3 mr-1" /> 마진 34%</div>
                </div>
                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                    <p className="text-sm font-bold text-gray-500 mb-1">객단가 (ARPU)</p>
                    <h3 className="text-2xl font-bold text-gray-900">71,800원</h3>
                    <div className="flex items-center mt-2 text-xs font-bold text-gray-500"><CreditCard className="w-3 h-3 mr-1" /> 유지</div>
                </div>
                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                    <p className="text-sm font-bold text-gray-500 mb-1">식재료 원가율</p>
                    <h3 className="text-2xl font-bold text-gray-900">32.8%</h3>
                    <div className="flex items-center mt-2 text-xs font-bold text-red-500"><ArrowDownRight className="w-3 h-3 mr-1" /> 목표 초과</div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[400px]">
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col">
                    ...
                    <div className="flex-1 min-h-0 relative">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={REVENUE_BY_CATEGORY} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
